...
import traceback
import logging
from Invest.analysis_today.except_hol import except_hol_cls

logging.basicConfig(level=logging.ERROR) # 로깅설정

class GBN2_cls:
    # name = 종목명, dd = 일자, q = 분기
    def Exe_GBN2(self,name,dd, q):
        # 엑셀 파일 열기
        ...
        for i in arr_yago:
            for k in arr_q:
                ...
                # 감가상각비
                try:
                    감가상각비 = data.loc[data['종목명'] == name][str(i)+'_감가상각비_'+str(k)].values[0]
                    if np.isnan(감가상각비) :
                        감가상각비 = data.loc[data['종목명'] == name][str(i)+'_감가상각비(2)_'+str(k)].values[0]
                except:
                    감가상각비 = 'non data'
                    
                # 퇴직급여
                try:
                    퇴직급여 = data.loc[data['종목명'] == name][str(i)+'_퇴직급여_'+str(k)].values[0]
                    if np.isnan(퇴직급여) :
                        퇴직급여 = data.loc[data['종목명'] == name][str(i)+'_퇴직급여(2)_'+str(k)].values[0]
                except:
                    퇴직급여 = 'non data'
                ...
                # 열 간격이 2, 행 간격이 1
                sheet.cell(31 + row_num_1, 4 + col_num_2, 자산총액)
                sheet.cell(41 + row_num_1, 5 + col_num_2, 유동자산)
                sheet.cell(46 + row_num_1, 5 + col_num_2, 매출채권)
                sheet.cell(51 + row_num_1, 5 + col_num_2, 재고자산)
                sheet.cell(59 + row_num_1, 4 + col_num_2, 부채총액)
                sheet.cell(69 + row_num_1, 5 + col_num_2, 유동부채)
                sheet.cell(74 + row_num_1, 5 + col_num_2, 전환사채)
                sheet.cell(79 + row_num_1, 5 + col_num_2, 신주인수권부사채)
                sheet.cell(84 + row_num_1, 5 + col_num_2, 상장주식수)
                sheet.cell(92 + row_num_1, 4 + col_num_2, 자본총액)
                sheet.cell(102 + row_num_1, 5 + col_num_2, 자본금)
                sheet.cell(107 + row_num_1, 5 + col_num_2, 자본잉여금)
                sheet.cell(112 + row_num_1, 5 + col_num_2, 이익잉여금)
                sheet.cell(125 + row_num_1, 4 + col_num_2, 매출액)
                sheet.cell(135 + row_num_1, 5 + col_num_2, 매출원가)
                sheet.cell(143 + row_num_1, 4 + col_num_2, 영업이익)
                sheet.cell(156 + row_num_1, 4 + col_num_2, 당기순이익)
                sheet.cell(166 + row_num_1, 5 + col_num_2, 비지배지분이익)
                sheet.cell(174 + row_num_1, 4 + col_num_2, 영업활동현금흐름)
                sheet.cell(184 + row_num_1, 5 + col_num_2, 감가상각비)
                sheet.cell(189 + row_num_1, 5 + col_num_2, 퇴직급여)
                sheet.cell(197 + row_num_1, 4 + col_num_2, 투자활동현금흐름)
                sheet.cell(210 + row_num_1, 4 + col_num_2, 재무활동현금흐름)
