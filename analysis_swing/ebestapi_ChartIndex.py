...
class ebestapi_ChartIndex_cls:
    ...
    def OnLogin(self, code, msg): # OnLogin으로 써야만 code, msg 정보를 리턴받을 수 있음
        if code == "0000":
            #print('login : ',msg)
            ebestapi_ChartIndex_cls.login_state = 1
        else:
            pass
            #print('login : ',msg)
...
class ChartIndex_cls:
    def exe_ChartIndex(self, shcode, edate):
        ...
        ## 로그인, 서버 연결
        # 객체 생성하기
        instXASession = win32com.client.DispatchWithEvents("XA_Session.XASession", ebestapi_ChartIndex_cls)
        ...
        # 수신(응답) 대기
        while ebestapi_ChartIndex_cls.login_state == 0:
