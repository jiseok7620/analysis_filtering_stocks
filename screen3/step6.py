...
class step6_cls:
    def make_step6(self, number, name, start, end, pm_day):
                #############################################################
        # 설치된 폰트정보보기
        ...
        start_idx_before = start_idx - 200
        end_idx_after = end_idx + 120
        
        # 차트에 텍스트를 표시하기위해 텍스트를 위치할 x와 y 구하기
        text_x_first = 200
        #text_x_last = 200 + (end_idx - start_idx)
        ...
        plt.text(text_x_first, text_y_first_min-(text_y_first_min/15), str(y_data_fluctuation_rate[text_x_first]), fontdict=font1) # 내가 구할 첫번째 봉에 등락률 표시
        ...
        # 2번 차트설정
        # 저장
        plt.savefig('F:/JusikData/analysis_csv/step/step6/'+name+'_'+start+'.png')
