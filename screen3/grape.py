...
class Grape_cls:
    ...
    def Make_Grape(self, name, start, end, pm_day):
        ...
        plt.text(text_x_last, text_y_last+(text_y_last/20), end, rotation=60) # 내가 구할 마지막 봉에 날짜 표시
        ...
        plt.text(text_x_last, text_y_last_min-(text_y_last_min/20), str(y_data_close[text_x_last]), fontdict=font1) # 내가 구할 마지막 봉에 종가 표시
        plt.axvline(text_x_first, color='black', linestyle='--', linewidth=1) # 내가 구할 첫번째 봉에 수직선 표시 
        plt.axvline(text_x_last, color='black', linestyle='--', linewidth=1) # 내가 구할 마지막 봉에 수평선 표시
        ...
        # 2번 차트설정
        # 최대거래량
        plt.text(0,1, 'Max : ' + str(max(y_data_trade)), fontdict=font2) # 내가 구할 첫번째 봉에 종가 표시
        # 최소거래량
        plt.text(text_x_last,1, 'Min : ' + str(min(y_data_trade)), fontdict=font2) # 내가 구할 마지막 봉에 종가 표시
        # 평균거래량
        plt.text(text_x_first,1, 'Avg : ' + str(round(np.mean(y_data_trade),1)), fontdict=font2) # 내가 구할 마지막 봉에 종가 표시
        
        # 차트보기
        #plt.show()
        
        # 차트저장하기
        #plt.savefig('F:/JusikData/analysis_csv/grape_img/'+name+'_'+start+'.png')
        
        # 테스트
        plt.savefig('F:/JusikData/analysis_csv/test/'+name+'_'+start+'.png')
...
'''
# 증가한 종목의 기준일 파일.csv      
increase_data = pd.read_csv('F:/JusikData/analysis_csv/increase/분석_증가데이터.csv', encoding='cp949')
...
for index, row in increase_data.iterrows():
    try:
        ...
        # 함수 실행
        conn.Make_Grape(row['종목명'], sl_date, end_day, 20)
    except:
...
# 증가한 종목의 기준일 파일.csv      
increase_data = pd.read_csv('F:/JusikData/analysis_csv/increase/분석_증가데이터2.csv', encoding='cp949')
...
for index, row in increase_data.iterrows():
    try:
        ...
        # 함수 실행
        conn.Make_Grape(row['종목명'], sl_date, end_day, 30)
    except:
        coment = row['종목명'] + sl_date + end_day + '_오류'
        print(coment)
        err.append(coment)
